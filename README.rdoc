...
React components don’t define transition between states. It only renders when it changes which makes it extremely responsive and eliminates the task of manually tweaking the DOM.  The React/Flux one-way data transfer structure makes is great for adding, deleting, and keeping track of tasks.
